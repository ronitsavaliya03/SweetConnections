...
import 'package:progress_dialog_null_safe/progress_dialog_null_safe.dart'; // Import ProgressDialog
...
class _AddEditScreenState extends State<AddEditScreen> {
  ...
  ValueNotifier<bool> _isLikedNotifier = ValueNotifier<bool>(false);
  ...
  Future<void> loadUserData() async {
    if (widget.data != null) {
      ...
      List<String> fetchedHobbies = hobbiesRaw
          .split(',')
          .map((h) => h.trim())
      ...
      _isLikedNotifier.value = (widget.data!['isLiked'] == '1'); // Assuming '1' means liked
    } else {
      _setDefaultDate();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.data!=null ? "Edit Candidate Details":
          "Add Candidate",
              ...
              _buildTextField(
                ...
                inputFormatters: [
                  FilteringTextInputFormatter.allow(RegExp(
                      r'^[\d,]*\.?\d*$')), // Only allow digits and commas
                ],
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () => _submitForm(context), // Pass the context
                        child: Text(
  ...
  Widget _buildTextField({
    ...
    return TextFormField(
      ...
      MaxLengthEnforcement.none, // Prevents blocking input
  ...
  void _selectDate() async {
    ...
    DateTime oldestSelectableDate =
    today.subtract(Duration(days: 80 * 365));
    ...
    return ValueListenableBuilder<bool>(
      ...
      builder: (context, isLiked, child) {
        return ElevatedButton.icon(
          onPressed: () {
            _isLikedNotifier.value = !isLiked; // Toggle the liked state
          },
  ...
  Future<void> _submitForm(BuildContext currentContext) async {
    ...
    if (_formKey.currentState!.validate()) {
      Map<String, dynamic> map = {
        ...
        PHONE: _mobileController.text.trim().toString(),
        CITY: _selectedCity.toString(),
        DOB: _dobController.text.trim().toString(),
        GENDER: _selectedGender.toString(),
        HOBBIES: _selectedHobbies.join(', ').toString(),
        OTH_HOBBIES: _otherHobbies.text.trim().toString(),
        EDUCATION: _educationController.text.trim().toString(),
        OCCUPATION: _occupationController.text.trim().toString(),
        WORK_PLACE: _workPlaceController.text.trim().toString(),
        INCOME: double.parse(_incomeController.text.replaceAll(',', '')).toString(),
        PASSWORD: _passwordController.text.toString(),
        CON_PASS: _confirmPasswordController.text.toString(),
        AGE: age.toString(),
        ISLIKED: _isLikedNotifier.value ? '1' : '0',
      };

      ProgressDialog pd = ProgressDialog(currentContext);
      pd.style(message: 'Saving...');

      try {
        // await apiService.showProgressDialog(context);

        dynamic response;

        if (widget.data == null) {
          response = await apiService.addUser(context: currentContext, map: map);
          await Future.delayed(Duration(milliseconds: 500));
        } else {
          final id = widget.data![ID];
          response = await apiService.updateUser(
              context: currentContext, id: id, map: map);
          await Future.delayed(Duration(milliseconds: 500));
        }
        setState(() {});
        print("success");
        ...
        if (widget.data != null) {
          ...
          Navigator.pop(currentContext, response);
        }
        ...
        await pd.hide(); // Ensure progress dialog is always hidden
      }
    }
  }
