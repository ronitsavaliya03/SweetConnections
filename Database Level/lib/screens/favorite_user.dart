...
class _FavoriteUserListState extends State<FavoriteUserList> {
  ...
  Future<void> fetchFavoriteUsers() async {
    final allUsers = await DatabaseHelper.instance.getAllUsers();
    setState(() {
      favoriteUsers = allUsers.where((user) => user[ISLIKED] == 1).toList();
    });
  }
                      ...
                      onSelected: (value) async {
                        if (value == 'edit') {
                          ...
                          if (updatedUser != null) {
                            ...
                            updatedUser[ISLIKED] = 1;

                            await DatabaseHelper.instance.updateUser(updatedUser); // Update the database
  ...
  Future<void> _deleteUser(int userId) async {
    ...
    setState(() {
      favoriteUsers.removeWhere((u) => u[ID] == userId);
    });
  }
}
